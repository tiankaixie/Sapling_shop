						...
						<div class="navigation-wrapper">
							<div class="navigation clearfix-normal">
								<ul class="nav">
									<li><a href="#">个人中心</a></li>
									<li><a href="#">我的收藏</a></li>
									<li><a href="my_orders">我提交的订单</a></li>
									<li><a href="#">修改密码</a></li>
									<c:if test="${loginUser.type=='1'}">
										<li class="active"><a class="active" href="store">我的店铺</a>
										</li>
										<li><a href="#">修改店铺信息</a></li>
										<li><a href="store_orders">店铺订单</a></li>
									</c:if>
								</ul>
								<!-- /.nav -->
				...
				<div id="content">
					<div class="container">
						<div id="main">
							<div class="row">
								<div class="span9">
									<h2 class="page-header">我上架的商品</h2>

									<div class="properties-grid">
										<div class="row">
											<c:if test="${empty my_goods.datas }">
												<p style="text-align: center">您还尚未上架商品！</p>
											</c:if>
											<c:forEach items="${my_goods.datas}" var="um">
												<div class="property span3">
													...
													<div class="location">${um.type }&nbsp;${um.varieties }
														<c:if test="${um.status==1 }">
															<span style="float: right; color: green">已上架</span>
														</c:if>
														<c:if test="${um.status==0 }">
															<span style="float: right; color: red">已下架</span>
														</c:if>
													</div>
													<!-- /.location -->
													<div class="area">
													...
													<div class="bedrooms">
														<c:if test="${um.status==1 }">
															<a href="itemDownshelf?id=${um.id}">下架商品</a>
														</c:if>
														<c:if test="${um.status==0 }">
															<a href="itemUpshelf?id=${um.id}">上架商品</a>
														</c:if>
														<a href="update_goods?id=${um.id}">修改</a> <a
															href="delete_goods?id=${um.id}">删除</a>
													</div>
													<!-- 弹出框 -->
													<div class="modal hide fade" id="itemUpshelf">
														<div class="modal-header">
															<a href="#" class="close" data-dismiss="modal">×</a>
															<h4>设定库存</h4>
														</div>
														<form class="control-horizontal" action="addStock"
															method="get">
															<div class="modal-body">
																...
																<div class="control-group">
																	<label class="control-label">预定库存</label>
									...
									<div class="pagination pagination-centered">
										<ul>
											<pg:pager maxPageItems="9" items="${my_goods.totalRecord}"
												export="curPage = pageNumber" url="store">

												<pg:param name="name" value="${search_name}" />
												<pg:last>
													<p>共${my_goods.totalRecord}条记录，共${pageNumber}页，当前第${curPage}页</p>
												</pg:last>
								...
								<div class="sidebar span3">
									<div class="widget our-agents">
										...
										<div class="content">
											<div class="agent">
												<h3>
													<strong>${my_store.name }</strong>
												</h3>
												<br> <strong>负责人：</strong>${loginUser.username }<br>
												<strong>手机：</strong>${my_store.phone }<br> <strong>手机：</strong>${my_store.email }<br>
												<strong>邮编：</strong>${my_store.postcode }<br> <strong>店家地址：</strong>${my_store.address }<br>
												<strong>店铺简介：</strong>${my_store.details }<br> <br />
